(function () {
  ...
  function isNationalPosition(id) { return data.positionScopes[id] === "national"; }
  ...
  function renderDetail() {
    ...
    const nationalPositionIds = positionIds.filter(isNationalPosition);
    const localPositionIds = positionIds.filter((id) => !isNationalPosition(id));
    candidateDetail.innerHTML = `
      ...
      <h4 class="positions-title">Θέσεις σε κοινά εθνικά ζητήματα</h4>
      ${nationalPositionIds.length ? nationalPositionIds.map((id) => renderPosition(id, true)).join("") : `<div class="empty-state compact">Δεν έχει καταχωριστεί ακόμη συγκρίσιμη θέση σε εθνικό ζήτημα.</div>`}
      ${localPositionIds.length ? `<details class="local-positions"><summary>Τοπικές παρεμβάσεις (${localPositionIds.length})</summary><p>Παρουσιάζονται για ενημέρωση, αλλά δεν επηρεάζουν την προσωπική σύγκριση.</p>${localPositionIds.map((id) => renderPosition(id, false)).join("")}</details>` : ""}`;
  }

  function renderPosition(id, comparable) {
    ...
    return `<section class="position-card ${comparable ? "" : "local-card"}">
      <div class="position-meta"><span>${position.pillar}</span></div>
      ...
      ${comparable ? `<div class="agreement">
  ...
  function renderResults() {
    const scored = districtCandidates(state.district).map((candidate) => {
      const answered = (candidate.positions || []).filter(isNationalPosition).map((id) => state.answers.get(id)).filter((value) => value !== undefined);
